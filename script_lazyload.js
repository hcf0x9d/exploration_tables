...
class TableIt {
    constructor( selector, headers, tableStyle ) {
        ...
        const element = document.getElementById( 'table_wrap' );
    ...
    addNewRows() {
        ...
        fetch("./data.json")
            ...
            .then( ( data ) => {
                ...
                setTimeout( ( ) => {
                    // Remove the loading row before adding the new data
                    _this.table.querySelector( '#loadingRow' ).remove();
                    data.forEach( ( set, idx ) => {
                        _this.buildTableRow( set, idx );
                    } );
                    _this.pause = false;
                }, _this.ajaxTime );
            })
